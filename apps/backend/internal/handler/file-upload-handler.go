...
func (h *FileUploadHandler) CreatePresignedUploadURL(c *gin.Context) {
	...
	if !isAllowedUploadContentType(ext, req.ContentType) {
		utils.ErrorResponse(c, http.StatusBadRequest, "BAD_REQUEST", "file content type is not allowed")
		return
	}
	...
	uploadURL, err := h.r2.PresignPutObject(c.Request.Context(), objectKey, req.ContentType, time.Duration(h.cfg.R2_PRESIGN_TTL_SECONDS)*time.Second)
	if err != nil {
	...
	utils.SuccessResponse(c, http.StatusAccepted, "file uploaded and queued for parsing", gin.H{
		...
		"expires_in":   h.cfg.R2_PRESIGN_TTL_SECONDS,
	}, nil)
...
func isAllowedUploadContentType(ext, contentType string) bool {
	...
	switch ext {
	case ".csv":
		return contentType == "text/csv" || strings.HasPrefix(contentType, "text/plain")
	case ".pdf":
